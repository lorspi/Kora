...
// Dual engine File System interface
...
/**
 * File system adapter providing a unified API for the real disk (FSA API) and IndexedDB Virtual Drive.
 */
export class FileSystemAdapter {
  private mode: FsMode = 'VIRTUAL';
  private rootDirectoryHandle: FileSystemDirectoryHandle | null = null;

  constructor(mode: FsMode = 'VIRTUAL', handle: FileSystemDirectoryHandle | null = null) {
    this.mode = mode;
  ...
  async readTextFile(path: string): Promise<string> {
    ...
    if (this.mode === 'VIRTUAL') {
      const record = await dbGet(normalized);
      if (!record) {
  ...
  async listFiles(subFolder: string): Promise<string[]> {
    ...
    if (this.mode === 'VIRTUAL') {
      ...
      // Filter keys that belong to this subfolder
      return allKeys
        .filter(key => key.startsWith(folderNormalized))
        .map(key => key.replace(folderNormalized, ''));
    } else {
      if (!this.rootDirectoryHandle) {
        return [];
      }
